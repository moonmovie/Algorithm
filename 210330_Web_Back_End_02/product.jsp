...
$(document).ready(function () {
        ...
        $('#submit').on("click",()=>{
           if($('#name').val()!=null && $('#price').val()!=null && $('#info').val()!=null){
               $('#productIn').attr('action','<%=root%>/product?act=productin').submit();
           }
        });
    });
</script>
	...
	<nav class="navbar navbar-expand-sm bg-dark navbar-dark">
		...
		<div class="collapse navbar-collapse" id="collapsibleNavbar">
			<ul class="navbar-nav">
				...
				<li class="nav-item"><a class="nav-link" href="<%=root%>/login?act=logout">LOGOUT</a></li>
				<li class="nav-item"><a class="nav-link" href="<%=root%>/product?act=productlist">PRODUCT
						LIST</a></li>
				<li class="nav-item"><a class="nav-link" href="<%=root%>/product?act=lastlist">ADDED
						LIST LAST</a></li>
			</ul>
		</div>
	</nav>
